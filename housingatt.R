...
AmesHouesingDataFrame <- read.csv("train.csv")
ui <- shinyUI(fluidPage(theme = shinytheme("cosmo"),
  navbarPage("Home Sale Price v. Ground Living Area"),
  tabsetPanel(
    ...
    tabPanel("Neighborhood Sale Price Scatterplot",
             ...
             h4("Zoning Class Key:",tags$br(),
       "A:  Agriculture",tags$br(),
       "C:	Commercial", tags$br(),
       "FV:	Floating Village Residential",tags$br(),
       "I:	Industrial",tags$br(),
       "RH:	Residential High Density",tags$br(),
       "RL:	Residential Low Density",tags$br(),
       "RP:	Residential Low Density Park",tags$br(),
  ...
  output$MyPlot <- renderPlot({
    ...
    return(plot)
       ...
       ggplot(data(), aes_string(x = input$xcol, y = input$ycol)) +
         ...
         guides(col = guide_legend(title = input$color))+
