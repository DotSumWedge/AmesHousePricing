...
Numerical Encoding so that we can explore correlation for LAND VAR
```{r}
...
setting up the correlation plot for creating mega variables
```{r}
# selecting var for exploration:
pool_corr <- c("LogSalePrice", "LogLotArea", "LotShapeNum",
               "LandContourNum", "LotConfigNum", "LandSlopeNum")

# getting the rows for each of the vars to be explored:
pool_df <- AmesHouesingDataFrame[, pool_corr]
pool_df <- cor(pool_df)
glimpse(pool_df)
# plotting corr plot with ggcorrplot
ggcorrplot(corr = pool_df, lab = TRUE, lab_size = 3,
           colors = c("#6D9EC1", "white", "#E46726")) +
