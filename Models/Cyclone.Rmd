...
# model from the paper submittal
```{r}
# FORWARD MODEL SELECTION
#ols_step_forward_p(home_reg2, prem = .05, details = FALSE)

# model_forward <- lm(LogSalePrice ~ OverallQual + LogGrLivArea + YearBuilt + LogLotArea +
#                        OverallCond + BedroomAbvGr + LogTotalBsmtSF + Fireplaces +
#                        LogGarageArea + HeatingQCNum + LogScreenPorch + LogWoodDeckSF, data = lr_model2)

model_forward <- lm(LogSalePrice ~ OverallQual + LogGrLivArea + YearBuilt + LogLotArea + OverallCond +
                      BedroomAbvGr + LogTotalBsmtSF + Fireplaces + LogGarageArea + HeatingQCNum + 
                      LogScreenPorch + LogWoodDeckSF + CentralAirNum + TotRmsAbvGrd + KitchenAbvGr + 
...
pred_r_squared(model_forward) # pred r2
...
pred_r_squared(model_bk) # pred r2
